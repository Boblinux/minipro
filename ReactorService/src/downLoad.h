    ...
    if(buf.readableBytes() >= 4+36+BlockFileSize)
    {
      ...
      if(g_File.lsFile("kvmap_2.txt"))
      {
        ...
        if(getstringMD5(FileContent.substr(36)) == FileContent.substr(0,32))
        {
          ...
          Bloc->prepend(&bsbe, 4);
          std::string str = Bloc->readAsString();
          //std::cout << str.size() << '\n';
          conn->send(str);*/
        }
              ...
              Reactor::Buffer* buf)
{
  while(buf->readableBytes()>=4)
  {
    ...
    const void *tmp = buf->peek();
    int32_t be32 = *static_cast<const int32_t*>(tmp);
    size_t FileNameLen = ::ntohl(be32);
    
    if(buf->readableBytes() >= 4+FileNameLen)
    {
      std::cout << FileNameLen << '\n';
      buf->retrieve(4);
      std::string FileName = buf->readAsBlock(FileNameLen);

      if(g_File.lsFile(FileName))
      {
        std::cout << FileName << " is in map" << '\n';
        //int num = g_File.getFileNum(FileName);
        //uint32_t bsbe = htonl(num);
        //Reactor::Buffer Buff;
        //Buff.append(g_File.getNumIdMD5(FileName).data(), g_File.getNumIdMD5("kvmap_2.txt").size());
        //Buff.prepend(&bsbe, 4);
        std::cout << g_File.getNumIdMD5(FileName);
        conn->send(g_File.getNumIdMD5(FileName));
      }
      else
      {
        conn->send("NOFile");
