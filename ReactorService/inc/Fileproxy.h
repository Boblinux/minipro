...
void setdownloadId(std::string FileName, int id)
{
    FileMap_[FileName]->setdownloadId(id);
}
...
std::string getNumIdMD5(std::string FileName)
{
    std::string str;
    int num = getFileNum(FileName);
    for(int i=0; i<num; i++)
    {
      str += getBlockFileMD5(i, "kvmap_2.txt").data();
    }
    return str;
}
